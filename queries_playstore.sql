...
SELECT id, app_name FROM analytics WHERE last_updates IN '2018-08-01';
-- Count the number of apps in each category, e.g. “Family | 1972”.
SELECT category, COUNT(*) FROM analytics GROUP BY category; 
-- Find the top 5 most-reviewed apps and the number of reviews for each.
SELECT * FROM analytics ORDER BY reviews DESC LIMIT 5;
-- Find the app that has the most reviews with a rating greater than equal to 4.8.
SELECT * FROM analytics WHERE reviews  = (SELECT MAX(reviews) FROM analytics WHERE rating >= 4.8);
...
SELECT * FROM analytics WHERE min_installs < 50 AND rating IS NOT NULL ORDER BY min_installs DESC; 
-- Find the names of all apps that are rated less than 3 with at least 10000 reviews.
SELECT * FROM analytics WHERE rating < 3 AND rating IS NOT NULL AND reviews <= 10000; 
...
SELECT app_name, reviews, min_installs,  min_installs / reviews AS proportion
  FROM analytics
  WHERE min_installs / reviews = (SELECT MAX (min_installs / reviews) FROM analytics WHERE min_installs > 100000);
